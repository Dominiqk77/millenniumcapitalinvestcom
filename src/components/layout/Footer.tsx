        ...
        <div className="grid gap-12 lg:grid-cols-5">
          <div className="lg:col-span-2">
            <a href="/" className="inline-flex items-center gap-3 group">
              ...
              <div>
                <span className="block text-xl font-medium text-foreground">Millennium Capital Invest</span>
            ...
            <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="h-4 w-4" />Wyoming, USA | Paris | Dakar</div>
          ...
          <div>
            <h4 className="text-sm font-medium uppercase tracking-wider text-foreground">Entreprise</h4>
            <ul className="mt-4 space-y-3">{footerLinks.entreprise.map((link) => (<li key={link.label}><a href={link.href} className="text-sm text-muted-foreground transition-colors hover:text-primary">{link.label}</a></li>))}</ul>
          </div>
          <div>
            <h4 className="text-sm font-medium uppercase tracking-wider text-foreground">Services</h4>
            <ul className="mt-4 space-y-3">{footerLinks.services.map((link) => (<li key={link.label}><a href={link.href} className="text-sm text-muted-foreground transition-colors hover:text-primary">{link.label}</a></li>))}</ul>
          </div>
          <div>
            <h4 className="text-sm font-medium uppercase tracking-wider text-foreground">Ressources</h4>
            <ul className="mt-4 space-y-3">{footerLinks.ressources.map((link) => (<li key={link.label}><a href={link.href} className="text-sm text-muted-foreground transition-colors hover:text-primary">{link.label}</a></li>))}</ul>
          </div>
        </div>
        <div className="divider-glow mt-12" />
        <div className="mt-8 flex flex-col items-center justify-between gap-4 sm:flex-row">
          <p className="text-sm text-muted-foreground">© {currentYear} Millennium Capital Invest LLC. {t('footer.rights')}.</p>
          <div className="flex items-center gap-6">
            ...
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="text-muted-foreground transition-colors hover:text-primary"><Linkedin className="h-5 w-5" /></a>
          </div>
        </div>
